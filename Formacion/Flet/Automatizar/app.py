...
def main(page:ft.Page):
    ...
    # Contoles para la vista de extracción de audio
    audio_input_text = ft.Text(value="No se ha seleccionado ninguna carpeta", size=14, color=ft.colors.BLUE_200)
    ...
    def extract_audio():
        try:
            if not state["audio_input_folder"]:
                audio_result_text.value = "Error: Selecciona una carpeta con videos"
                audio_result_text.color = ft.colors.RED_400
                audio_result_text.update()
                return
            
            input_foder = state["audio_input_folder"]
            output_folder = os.path.join(input_foder, "audios")
            ...
            extraer_audio(input_foder, output_folder, progress_callback)
            ...
            audio_progress.visible = False  # Ocultar la barra de progreso al finalizar
            audio_progress.update()
    ...
    # Controles para la vista de fusión PDFs
    pdf_input_text = ft.Text(value="No se ha seleccionado ninguna carpeta para PDFs", size=14, color=ft.colors.BLUE_200)
    ...
    def merge_pdfs():
        try:
            ...
            pdf_result_text.value = f"PDFs fusionados correctamente en: {output_file}"
    ...
    rename_input_text = ft.Text(value="Carpeta de entrada: No seleccionada", size=14, color=ft.colors.BLUE_200)
    ...
    rename_option_dropdown = ft.Dropdown(label="Opción de renombrado", 
                                         options=[
    ...
    rename_search_text = ft.TextField(label="Palabra a buscar", width=200, visible=True)
    rename_replace_text = ft.TextField(label="Reemplazar por", width=200, visible=True)
    
    rename_prefix_text = ft.TextField(label="Prefijo a agregar", width=200, visible=False)
        ...
        try:
            ...
            if option == "cambiar":
                if not rename_search_text.value:
                    rename_result_text.value = "Error: Introduce la palabra a buscar"
                    rename_result_text.color = ft.colors.RED_400
                    rename_result_text.update()
                    return
                ...
                if not rename_prefix_text.value:
                    rename_result_text.value = "Error: Introduce el prefijo"
                    rename_result_text.color = ft.colors.RED_400
                    rename_result_text.update()
                    return
            ...
            rename_result_text.value = "Archivos renombrados correctamente"
        ...
        content=ft.Column([
            ...
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Text(value="* PDFs  (.pdf)", size=14),
                    ft.Text(value="* Documentos  (.doc, .docx, .xls, .xlsx, .ppt, .pptx, .txt)", size=14),
    ...
    convert_images_view = ft.Container(
        content=ft.Column([
            ft.Container(
                content=ft.Text("Convertir Formato de Imagen", size=28, weight=ft.FontWeight.BOLD, color=ft.colors.BLUE_200),
                margin=ft.margin.only(bottom=20),
                ),
        ft.ElevatedButton(
    ...
    extract_audio_view = ft.Container(
        content=ft.Column([
            ft.Container(
                content=ft.Text(value="Extraer Audio de Videos", size=28, weight=ft.FontWeight.BOLD, color=ft.colors.BLUE_200),
                margin=ft.margin.only(bottom=20)
                ),
            ft.ElevatedButton(
            ...
            ft.Container(
                content=ft.Column([
                    ft.Text(value="Información", size=14, color=ft.colors.BLUE_200),
                    ft.Text(value="* Formatos soportados. MP4, AVI, MOV, MKV",size=14),
                    ft.Text(value="* Los archivos de audio se extraerán en formato MP3",size=14),
                    ft.Text(value="* Los audios extraídos se guardarán en una carpeta 'audios2 dentro de la carpeta seleccionada",size=14),
    ...
    merge_pdfs_view = ft.Container(
        content=ft.Column([
                ft.Container(
                    content=ft.Text(value="Fusionar Archivos PDF", size=28, weight=ft.FontWeight.BOLD, color=ft.colors.BLUE_200),
                    margin=ft.margin.only(bottom=20)
                    ),
                ft.ElevatedButton(
    ...
    rename_files_view = ft.Container(
        content=ft.Column([
            ft.Container(
                content=ft.Text(value="Renombrar Archivos", size=28, weight=ft.FontWeight.BOLD, color=ft.colors.BLUE_200),
                margin=ft.margin.only(bottom=20)
                ),
            ft.ElevatedButton(
                text="Seleccionar Carpeta a renombrar",
                icon=ft.icons.FOLDER_OPEN,
                color=ft.colors.WHITE,
                bgcolor=ft.colors.BLUE_900,
                on_click=lambda _: folder_picker.get_directory_path()
