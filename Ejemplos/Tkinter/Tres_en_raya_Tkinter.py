...
def make_move(row, col):
    if board[row][col] == ' ':
        ...
        if check_winner(board, 'X'):
            messagebox.showinfo("Tres en Raya", "Tu ganas!")
            root.quit()
        elif is_board_full(board):
            messagebox.showinfo("Tres en Raya", "Esto es todo!")
            root.quit()
        else:
            ai_move()
    else:
        messagebox.showerror("Error", "Movimiento incorrecto")
...
def ai_move():
    ...
    if check_winner(board, 'O'):
        messagebox.showinfo("Tres en Raya", "AI gana!")
        root.quit()
    elif is_board_full(board):
        messagebox.showinfo("Tres en Raya", "Esto es todo!")
        root.quit()
