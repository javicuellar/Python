...
#  Este módulo agrupa las funciones que se realizan con tablas, BD SQLite.
#
#    - InsertaRegistro(lista).- inserta en la tabla Mercado la lista (acción, fecha, valor).
#    - LeerAccion(accion).- devuelve tupla (fecha,valor) de los registros de la tabla Mercado de la acción 
#    - CerrarTabla().- cierra la BD SQLite


import Mercado.analisis as a
#  Este módulo agrupa las funciones de análisis de datos de los valores
#
...
#  Función GrabarDatosFichero(fichero).- toma como parámetro un string con el nombre del fichero con
...
def GrabarDatosFichero(fichero):
	...
	leidoant = leido
	while leido[0] != '':
		leido = f.LeerFichero(fichero)	# lectura de valores desde fichero [acción, fecha, valor]
		if leido[0] != '':
			contador += 1
			...
			if leido[1] != leidoant[1]:		# no grabamos registro si está duplicado por fecha
				t.InsertaRegistro(leido)	# graba en BD SQLite en tabla Mercado [acción, fecha, valor]
				leidoant = leido
...
#  Función RecuperarValor(accion).- toma como parámetro un string con el nombre de la acción 
...
#  Función RecuperarValores().- Accede a la tabla Mercado y devuelve un diccionario con clave la acción y 
...
#  Función AnalizarValores(valores).- Realiza un análisis de la cartera enviada como parámetro (tipo diccionario)
